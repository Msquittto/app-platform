...
export function guestModeStopInstance(tenantId: string, instanceId: string, params: any) {
  ...
  return put(`${AIPP_URL}/guest/${tenantId}/instances/${instanceId}/terminate`, params);
}

// 获取会话历史对话
export function getGuestModeChatRecentLog(tenantId, chatId, appId) {
  return get(`${AIPP_URL}/guest/${tenantId}/log/app/${appId}/chat/${chatId}`);
}

// 用户自勾选删除历史记录
export function guestModeClearChat(appId, list) {
  return del(`${AIPP_URL}/guest/${appId}/log/logs`, list);
}
...
export function queryGuestModeDepartMent(tenantId, appId, debug = false) {
  return get(`${AIPP_URL}/guest/${tenantId}/app/${appId}/prompt?isDebug=${debug}`);
}

// 添加灵感大全
export function guestModeAddInspiration(tenantId, appId, id, data) {
  return post(`${AIPP_URL}/guest/${tenantId}/app/${appId}/prompt/${id}`, data);
}
...
export function guestModeDeleteInspiration(tenantId, appId, categoryId, id) {
  return del(`${AIPP_URL}/guest/${tenantId}/app/${appId}/prompt/${categoryId}/inspiration/${id}`);
}

// 获取灵感大全列表数据
export function queryGuestModeInspiration(tenantId, appId, categoryId, debug = false) {
  return get(`${AIPP_URL}/guest/${tenantId}/app/${appId}/prompt/${categoryId}?isDebug=${debug}`);
}
...
export function clearGuestModeInstance(tenantId, appId, type) {
  return del(`${AIPP_URL}/guest/${tenantId}/log/app/${appId}?type=${type}`);
}

// 多文件上传
export function guestModeUploadMultipleFile(tenantId, appId, data) {
  return post(`${AIPP_URL}/guest/${tenantId}/files?app_id=${appId}`, data, {
